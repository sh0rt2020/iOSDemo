...
- (id) initWithFrame:(CGRect)frame
             decoder: (AVCodecContext *) decoder {
    ...
    if (self) {
        ...
        if (decoder->pix_fmt == AV_PIX_FMT_YUV420P || decoder->pix_fmt == AV_PIX_FMT_YUVJ420P) {
            ...
            NSLog(@"use YUV GL renderer");
            
        } else {
            //RGB
            _renderer = [[KxMovieGLRenderer_RGB alloc] init];
            NSLog(@"use RGB GL renderer");
        }
...
- (void)updateVertices
{
    ...
    const float width   = _decoder->width;
    const float height  = _decoder->height;
